...
const AlumniDirectoryFixed = () => {
  ...
  const fetchAlumni = async () => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        setTotalPages(Math.ceil((data.data.total || 0) / 12));
      }
    } catch (error) {
  ...
  const applyFilters = () => {
    ...
    Object.keys(filters).forEach(key => {
      if (filters[key]) {
        filtered = filtered.filter(alum => {
          const alumValue = alum[key] ? alum[key].toLowerCase() : '';
        ...
        <div className="space-y-4">
          ...
          <div className="flex space-x-4 mt-4">
            <button
              ...
              className="flex items-center px-4 py-2 bg-mvsr-600 text-white rounded-lg hover:bg-mvsr-700 transition-colors"
            >
            ...
            <div className="bg-white rounded-lg shadow-md p-6 mb-6">
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center space-x-4">
                  <button
                    onClick={() => navigate('/alumni-profile/' + JSON.parse(localStorage.getItem('user')).id}
...
export default AlumniDirectoryModern;
